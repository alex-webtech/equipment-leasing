...
from .models import LessorDetails, LesseeDetails, ContainerDetails, LeasingDetails

# Create your views here.

# Main page
def ContainerHome(request):
    return render(request, 'containerhome.html')
...
def adminlogin(request):
    if request.method == 'POST':
        ...
        if username == 'alex' and password == 'admin':
...
def admin_lessorpending(request):
    ...
    return render(request, 'lessorpending.html', {'lessorpendingdata': lessorpending})
...
def lessor_register(request):
    if request.method == 'POST':
        ...
        if len(firstname) == 0 or len(lastname) == 0 or len(email) == 0 or len(phonenumber) == 0 or len(country) == 0 or len(state) == 0 or len(city) == 0 or len(username) == 0 or len(password) == 0 or len(confirmpassword) == 0:
            ...
            lessor.Email = email
            lessor.Phone_Number = phonenumber
...
def lessor_editprofile(request):
    ...
    return render(request, 'lessoreditprofile.html', {'lessordetail': lessordetail})
...
def lessor_editprofile_edit(request, id):
    ...
    if request.method == 'POST':
        ...
        city = request.POST.get('city')
        password = request.POST.get('password')
        if len(phonenumber) == 0 or len(country) == 0 or len(state) == 0 or len(city) == 0 or len(password) == 0:
            return HttpResponse('Please fill the empty fields')
        else:
            lessordata.Phone_Number = phonenumber
            lessordata.Country = country
            lessordata.State = state
            lessordata.City = city
            lessordata.Password = password
            lessordata.save()
            return redirect('/lessor_editprofile')
    return render(request, 'lessoreditprofile.html',{'lessordetail': lessordetail, 'data': lessordata})
...
def lessor_updatecontainer(request):
    ...
    return render(request, 'lessorupdatecontainer.html', {'containerdetail': containerdetail})
...
def lessor_updatecontainer_update(request, id):
    ...
    if request.method == 'POST':
        ...
        if len(quantity) == 0 or len(containeramount) == 0:
            ...
            updcontainer.save()
            return redirect('/lessorupdatecontainer')
    return render(request, 'lessorupdatecontainer.html',{'containerdetail': containerdetail, 'updcontainer': updcontainer})

def lessor_view_leased_container(request):
    leasedcontainer = LeasingDetails.objects.filter(Status=True)
    return render(request, 'lessorviewleasedcontainer.html',{'leasedcontainer': leasedcontainer})
...
def lessee_register(request):
    if request.method == 'POST':
        ...
        if len(firstname) == 0 or len(lastname) == 0 or len(email) == 0 or len(phonenumber) == 0 or len(company) == 0 or len(country) == 0 or len(state) == 0 or len(city) == 0 or len(username) == 0 or len(password) == 0 or len(confirmpassword) == 0:
            return HttpResponse('Please fill the all fields')
        elif password != confirmpassword:
            return HttpResponse('password is mismatch')
        else:
            ...
            lessee.Email = email
            lessee.Phone_Number = phonenumber
...
def lessee_log_reg(request):
    return redirect('/lesseeregister')
...
def lessee_editprofile(request):
    ...
    return render(request, 'lesseeeditprofile.html', {'lesseedetail': lesseedetail})
...
def lessee_editprofile_edit(request, id):
    ...
    if request.method == 'POST':
        ...
        if len(phonenumber) == 0 or len(country) == 0 or len(state) == 0 or len(city) == 0 or len(password) == 0:
            ...
            lesseedata.Phone_Number = phonenumber
    ...
    return render(request, 'lesseeeditprofile.html',{'lesseedetail': lesseedetail, 'data': lesseedata})

def lessee_view_container_list(request):
    containerdata = ContainerDetails.objects.filter(Status=True)
    return render(request, 'lesseeviewcontainerlist.html', {'containerdata': containerdata})
...
def lessee_lease_container(request, id):
    ...
    return render(request, 'lesseeviewcontainerlist.html', {'containerdata': containerdata, 'leasecontainer': leasecontainer})

def lessee_view_leasing_list(request):
    leasingdata = LeasingDetails.objects.filter(Status=True)
    return render(request, 'lesseeviewleasinglist.html', {'leasingdata': leasingdata})